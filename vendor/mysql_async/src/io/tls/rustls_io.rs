...
impl Endpoint {
    pub async fn make_secure(&mut self, domain: String, ssl_opts: SslOpts) -> Result<()> {
        ...
        let user_roots = ssl_opts.load_root_certs().await?;
        let mut root_store = RootCertStore::empty();
        // DBWarp Blueprint patch: when the caller supplies explicit root
        // certificates, trust ONLY those roots. Upstream mysql_async appends
        // user roots to webpki_roots, which violates DBWarp Blueprint's
        // restrictive --tls-ca contract.
        if user_roots.is_empty() {
            root_store.extend(webpki_roots::TLS_SERVER_ROOTS.iter().map(|x| x.to_owned()));
        }

        for cert in user_roots {
            root_store.add(cert)?;
        }
